"""
Stage 5 tests for the debate engine. Offline: a fake model returns canned replies, so no
quota is used.

What they protect:
- every role sees exactly the shared, sanitized case file, and nothing label-related
- the roles run in order and each builds on the previous arguments
- an interrupted debate resumes without repeating finished calls
- the Judge's reply is validated, and its HIGH level is the baseline's pre-registered band
...
CANNED_REPLIES = {
    ...
    "judge": {"decision": "approve", "reasoning": "D1 answers P1.", "verbalized_confidence": "medium",
              "confidence_justification": "One point is not fully answered."},
}
...
def test_roles_run_in_order_and_build_on_earlier_arguments():
    ...
    assert [role for role, _ in model.calls] == ["prosecutor", "defender", "judge"]
    ...
    assert prosecutor_json in user_messages["defender"]
    assert prosecutor_json in user_messages["judge"] and defender_json in user_messages["judge"]

    assert result["decision"] == "APPROVE" and result["verbalized_confidence"] == "MEDIUM"
    assert result["usage"]["total"]["total_tokens"] == 45 and result["attempts"]["total"] == 3
...
def test_resume_skips_finished_steps():
    ...
    saved = {}
    debate_agent.run_debate(claim, evidence, llm=first_model, on_step=lambda steps: saved.update(steps))
    ...
    assert [role for role, _ in resumed_model.calls] == ["defender", "judge"]


def test_judge_reply_is_validated():
    good = {"decision": "DENY", "reasoning": "r", "verbalized_confidence": "HIGH", "confidence_justification": "j"}
    for key, bad_value in [("decision", "MAYBE"), ("verbalized_confidence", "VERY HIGH"),
...
def test_baseline_and_every_debate_role_share_the_same_guide_and_review_guidance():
    for prompt in (BASELINE_PROMPT, debate_agent.PROSECUTOR_SYSTEM_PROMPT,
                   debate_agent.DEFENDER_SYSTEM_PROMPT, debate_agent.JUDGE_SYSTEM_PROMPT):
